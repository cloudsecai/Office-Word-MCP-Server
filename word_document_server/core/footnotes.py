...
def get_format_symbols(numbering_format: str, count: int) -> List[str]:
    ...
    if numbering_format == "i, ii, iii":
        ...
        return roman_numerals[:count] + [
            str(i)
            for i in range(count - len(roman_numerals) + 1, count + 1)
        ...
        return alphabet[:count] + [
            str(i)
            for i in range(count - len(alphabet) + 1, count + 1)
            if i > len(alphabet)
        ]
        ...
        return symbols[:count] + [
            str(i)
            for i in range(count - len(symbols) + 1, count + 1)
            if i > len(symbols)
        ]
...
def customize_footnote_formatting(
    doc, footnote_refs, format_symbols, start_number, style=None
):
    """
