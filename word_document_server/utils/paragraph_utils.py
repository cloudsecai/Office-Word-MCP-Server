...
def set_paragraph_text_util(doc_path: str, paragraph_index: int, new_text: str, 
                           style_to_apply: Optional[str] = None) -> Dict[str, Any]:
    """
...
def insert_paragraph_after_index_util(doc_path: str, target_paragraph_index: int, text_to_insert: str, 
                                     style_to_apply: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not (0 <= target_paragraph_index < len(doc.paragraphs)):
            return {"error": f"Invalid paragraph index: {target_paragraph_index}. Document has {len(doc.paragraphs)} paragraphs."}
